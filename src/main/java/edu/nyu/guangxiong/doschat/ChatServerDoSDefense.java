...
public class ChatServerDoSDefense implements Runnable {

    private static final int MAXIMUM_WORKLOAD = 16;

    private static final double THRESHOLD_ACCEPT_RATIO = 0.5;
    ...
    public static void main(String[] args) throws IOException {
        System.out.println("A DoS Defensive Chat Server is ON, waiting for clients to chat...");
        ChatServerDoSDefense server = new ChatServerDoSDefense();
        server.run();
    }
    ...
    public ChatServerDoSDefense() throws IOException {
    ...
    private void process() throws IOException {
		...
		// Get all idle channels since last invoke of select().
        Set<SelectionKey> idleKeys = new HashSet<>();
        idleKeys.addAll(selector.keys());
		idleKeys.removeAll(SelectedKeys);
		Iterator<SelectionKey> iteratorIdleKey = idleKeys.iterator();

        while (iteratorSelectedKey.hasNext()) {
            SelectionKey key = iteratorSelectedKey.next();

            // Limit the number of accepted sockets.
            if (key.isAcceptable() && selector.keys().size() <= Math.round(MAXIMUM_WORKLOAD * THRESHOLD_ACCEPT_RATIO)) {
                System.out.println("another chat client is accepted");
                SocketChannel client = channel.accept();
                client.configureBlocking(false);

                // Track idle time of each registered socket channel.
				final Long idleTimeTracker = System.currentTimeMillis();
                client.register(selector, SelectionKey.OP_READ);
                writeBuffers.put(client, ByteBuffer.allocate(READ_BUFFER_SIZE));
            }
            else if (key.isReadable()) {
                // Update with current time only when the Chat client has input to ChatServer.
                System.out.println("another chat client is being read...");
				final Long idleTimeTracker = System.currentTimeMillis();
				key.attach(idleTimeTracker);
        ...
        // Scan each idle channel to check its elapsed time and cancel its registration if exceeds threshold.
		while (iteratorIdleKey.hasNext()) {
			SelectionKey key = iteratorIdleKey.next();
			if (key.isReadable()) {
				System.out.println("some non-server-socket channels are idle~");
				long elapsed = selectTime - (long)key.attachment();
				if (elapsed >= THRESHOLD_IDLE) {
					System.out.println("a idle channel is cancelled");
					key.cancel();
					// The cancelled client will not receive chats.
					writeBuffers.remove(key.channel());
				}
			}
			iteratorIdleKey.remove();
		}
    }
}
